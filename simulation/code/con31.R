...
#1st continuous generation model; medium effect size; create 20% MCAR in x1 and x2 and create 20% MAR in Y (depends on x9 and x10)
...
sim<-function(Sd=1,runs=10,M=20,seedset=SEED){
  ...
  for(r in 1:runs){
    ...
    lm.fit<-lm(y ~ x1+x2+x3+x8+x9+I(x3^2),data=complete.df)
    ...
    #create 20% MCAR in x1 and x2
    withNA.df<-createNA(withNA.df,names=c("x1","x2"),p=c(0.2,0.2))
    ...
    mice.pmm<-with(imputed.list,lm(y ~ x1+x2+x3+x8+x9+I(x3^2)))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.midastouch<-with(imputed.list,lm(y ~ x1+x2+x3+x8+x9+I(x3^2)))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.cart<-with(imputed.list,lm(y ~ x1+x2+x3+x8+x9+I(x3^2)))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.rf<-with(imputed.list,lm(y ~ x1+x2+x3+x8+x9+I(x3^2)))
    ...
    missranger<-with(imputed.list,lm(y ~ x1+x2+x3+x8+x9+I(x3^2)))
    ...
    mixgb.pmm0<-with(imputed.list,lm(y ~ x1+x2+x3+x8+x9+I(x3^2)))
    ...
    mixgb.pmm1<-with(imputed.list,lm(y ~ x1+x2+x3+x8+x9+I(x3^2)))
    ...
    mixgb.pmm2<-with(imputed.list,lm(y ~ x1+x2+x3+x8+x9+I(x3^2)))
...
con31<-sim(Sd=1,runs=1000,M=20,seedset=SEED)
save(con31,file="/home/simulation/result/con31.RData")
