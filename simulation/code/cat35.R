...
#2nd categorical generation model; large effect size;create 20% MCAR in d1 and d2 and create 20% MAR in Y (depends on x9 and x10)
...
sim<-function(runs=10,M=20,seedset=SEED){
  ...
  for(r in 1:runs){
    ...
    #create 20% MCAR in d1 and d2
    withNA.df<-createNA(withNA.df,names=c("d1","d2"),p=c(0.2,0.2))


    all.col<-names(complete.df)
    complete.df[,all.col]<-lapply(complete.df[,all.col],factor)

    glm.fit<-glm(y ~ d1+d2+d3+d4+d8+d9+d3:d4,data=complete.df,family=binomial(link = "logit"))
    ...
    mice.logreg<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d3:d4,family=binomial()))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.logregboot<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d3:d4,family=binomial()))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.cart<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d3:d4,family=binomial()))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.rf<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d3:d4,family=binomial()))
    ...
    missranger<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d3:d4,family=binomial()))
    ...
    mixgb.pmm0<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d3:d4,family=binomial()))
    ...
    mixgb.pmm1<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d3:d4,family=binomial()))
    ...
    mixgb.pmm2<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d3:d4,family=binomial()))
...
cat35<-sim(runs=1000,M=20,seedset=SEED)
save(cat35,file="/home/simulation/result/cat35.RData")
