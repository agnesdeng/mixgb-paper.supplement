...
#3rd categorical generation model; large effect size; create 20% MCAR in d1 and d2 and create 20% MAR in Y (depends on x9 and x10)
...
sim<-function(runs=10,M=20,seedset=SEED){
  ...
  for(r in 1:runs){
    ...
    glm.fit<-glm(y ~ d1+d2+d3+d4+d8+d9+d8:d9,data=complete.df,family=binomial(link = "logit"))
    ...
    mice.logreg<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d8:d9,family=binomial()))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.logregboot<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d8:d9,family=binomial()))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.cart<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d8:d9,family=binomial()))
    ...
    a<-as.list(a)
    imputed.list<-imputationList(a)
    mice.rf<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d8:d9,family=binomial()))
    ...
    missranger<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d8:d9,family=binomial()))
    ...
    mixgb.pmm0<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d8:d9,family=binomial()))
    ...
    mixgb.pmm1<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d8:d9,family=binomial()))
    ...
    mixgb.pmm2<-with(imputed.list,glm(y~d1+d2+d3+d4+d8+d9+d8:d9,family=binomial()))
...
cat36<-sim(runs=1000,M=20,seedset=SEED)
save(cat36,file="/home/simulation/result/cat36.RData")
