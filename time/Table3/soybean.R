...
data("Soybean")
str(Soybean)
dim(onehot(Soybean)$onehot.df)
M=5
s=2021
set.seed(s)

withNA.df<-createNA(Soybean,p=0.3)


###midastouch doesn't work
#if doesn't set scale_pos_weight="auto", default is 1. errors may occurs for xgboost because of imbalanced variables
soybean.time<-microbenchmark("mice.default"={imp<-mice(withNA.df,m=M,seed=s,print=T)},
...
soybean.time
save(soybean.time,file="/Users/agnes/Desktop/MIXGB-jcgs/Supplement/time/Table3/result/soybean.RData")
